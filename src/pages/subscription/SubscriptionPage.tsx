    ...
    <div className="container max-w-3xl py-10">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <PlanCard
          name="Free"
          price="$0"
          features={["5 try-ons/month", "Standard processing", "Basic AI models"]}
        />
        <PlanCard
          name="Pro"
          price="$9.99/mo"
          features={["Unlimited try-ons", "Faster processing", "Premium models", "Priority support"]}
          popular
        />
        <PlanCard
          name="Business"
          price="$24.99/mo"
          features={["Everything in Pro", "Team access (seats)", "Commercial rights", "API access"]}
        />
      </div>
...
function PlanCard({
  ...
  return (
    <div className={`border rounded-xl bg-card p-4 ${popular ? "border-accent" : "border-border"}`}>
      {popular && (
        <span className="text-xs bg-accent text-accent-foreground px-2 py-0.5 rounded">Popular</span>
      )}
